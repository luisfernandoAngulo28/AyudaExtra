 ...
 * @author Usuario
 */
public class ListaDGRep <T extends Comparable> {
 ...
 private String mostrarReves(NodoDG<T> nodo){
    ...
    public String toString() {
     ...
     public NodoDG<T> EliminarRep(NodoDG<T> p)
     {
      if(p!=null&p.getEnlaceD()!=null)
      {
       p.setEnlaceD(EliminarRep(p.getEnlaceD()));
       T aux= p.getDato();
       if(aux.compareTo(p.getEnlaceD().getDato())==0)
       {
       return p.getEnlaceD();
       }
      }
     ...
     {
     prim.setEnlaceD(EliminarRep(prim.getEnlaceD()));
     }
